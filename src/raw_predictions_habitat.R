...
source('./code/src/validation/functions_valid.R')

# Prepare data for interpolation
set.seed(123)
dat.i <- 
  bind_rows(
    read_csv('./code/data/input/EVA.csv', show_col_types = F), # Load EVA data
    format_ReSurveyEurope(
      training_strategy = 'random',
      path_resurvey_clean = './code/data/input/ReSurveyEU_clean.csv')[['traintest_data']]  # Load ReSurveyEU (static, using one random point in the survey)
  ) 
...
for(ind.name in ind.names){
  ...
  output_file <- paste0('./code/data/preds/pdp/', ind.name, '_habitat_pdp_trends_alltrees.csv')

  # Load model and extract workflow
  model_path <- paste0('./code/data/models/RF.last_fit_', ind.name, '.rds')
