...
| **How to obtain access** | Data can be accessed through a request to the Governing Board, following [Article 5 of the EVA rules](https://euroveg.org/download/eva-rules.pdf). More info: <https://euroveg.org/eva-database/obtaining-data> | Through this repository, in .csv.xz format ([`data`](data)). **To use these data elsewhere, a data request must be submitted to the Governing Board**. |
